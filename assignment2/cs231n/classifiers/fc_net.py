    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:  # last layer
                ...
                if self.normalization == 'batchnorm':  # forward with bn
                    ...
                    hidden_out[str(i + 1)], cache[str(i)] = affine_bn_relu_forward(hidden_out[str(i)], W, b, gamma,
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:  # output layer
                dhidden[str(i - 1)], grads['W' + str(i)], grads['b' + str(i)] = affine_backward(dhidden[str(i)],
                                                                                                cache[str(i - 1)])

            else:  # not out put layer
                if self.normalization == 'batchnorm':  # backward with bn
                    dhidden[str(i - 1)], grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads[
                        'beta' + str(i)] = affine_bn_relu_backward(dhidden[str(i)], cache[str(i - 1)])

                else:  # backward normally
                    dhidden[str(i - 1)], grads['W' + str(i)], grads['b' + str(i)] = affine_relu_backward(
                        dhidden[str(i)], cache[str(i - 1)])
